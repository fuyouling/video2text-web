...
export function detectPlatform(): PlatformInfo {
  ...
  if (platformHint.includes("win") || uaLower.includes("windows")) os = "windows";
    ...
    os = "macos";
  else if (platformHint.includes("linux") || uaLower.includes("linux") || uaLower.includes("x11"))
    os = "linux";
  ...
  if (uaLower.includes("arm64") || uaLower.includes("aarch64") || platformHint.includes("arm")) {
    ...
    uaLower.includes("x86_64") ||
    uaLower.includes("win64") ||
    uaLower.includes("x64") ||
    uaLower.includes("wow64")
  ) {
    arch = "x64";
...
export function pickAsset(assets: PickAsset[], info: PlatformInfo): PickAsset | null {
  ...
  return (
    assets.find((a) => a.arch === arch && a.kind === "installer") ??
    assets.find((a) => a.arch === arch) ??
    assets[0] ??
    null
  );
}
