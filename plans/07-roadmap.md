...
| **P1** | 静态官网上线 | 首页、下载页、功能页、定价（静态）、文档/博客框架、隐私/条款/退款 | 英语优先站点部署到 Cloudflare Pages，死链检查 + Lighthouse 达标 |
...
- [ ] 组件：`Header`/`Footer`/`Hero`/`FeatureCard`/`Workflow`/`CTASection`/`DownloadButton`/`LangSwitch`/`PricingCard`
...
- [ ] `/download`（静态兜底版本号 1.0 起步，P2 再接动态）
- [ ] `/features` 详细分点 + 占位截图
...
- [ ] 下载页接入 GitHub Release 动态版本（构建期注入 + 定时重建）；`/changelog` 版本日志页（Release 列表注入）
- [ ] 填充卖点文案、截图/动图、教程博客
